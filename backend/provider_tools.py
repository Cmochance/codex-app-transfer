...
def model_endpoint_candidates(provider: dict) -> list[str]:
    ...
    不同国产 API 对 OpenAI/Responses 兼容层的 URL 约定不同，所以这里按常见
    端点做少量候选，逐个尝试。
    """
    ...
    if api_format == "openai_chat":
        ...
        candidates.append(_replace_path_suffix(upstream, ("/v1/responses", "/responses"), "/v1/models"))
        if base_url.lower().endswith("/v1"):
            candidates.append(f"{base_url}/models")
        candidates.append(f"{base_url}/models")
        parts = urlsplit(base_url)
        stripped_path = parts.path.rstrip("/")
        if stripped_path.lower().endswith("/anthropic") or stripped_path.lower().endswith("/v1"):
            root_path = stripped_path[: -len("/anthropic")] if stripped_path.lower().endswith("/anthropic") else stripped_path[: -len("/v1")]
...
def extract_model_ids(payload: Any) -> list[str]:
    """从 OpenAI/Responses/通用模型列表响应中提取模型 ID。"""
    candidates: list[Any] = []
