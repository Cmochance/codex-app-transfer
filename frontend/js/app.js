(function () {
  ...
  // OpenAI provider),也可能是带元数据的 object(Antigravity 上游
  // `/api/antigravity-oauth/models` 的 data[] entry,含 display_name /
  // recommended / tag_title)。下面三个 helper 统一抽取,**只影响展示文本 /
  // 排序 / 标记,绝不改提交的 value** —— value 永远是 raw id。
  ...
  function modelEntryDisplayLabel(entry) {
    ...
    // 其他 provider 的 entry(string 或无 display_name 的 object)自动回退到 id,
    // 不破坏现有 gemini-cli / OpenAI provider 的纯 id 展示。
    if (typeof entry === "string") return entry;
    if (!entry || typeof entry !== "object") return "";
  ...
  function modelEntryIsRecommended(entry) {
    // 仅带 recommended 字段的 object 为 true;string / 无字段 object 视为非推荐。
    return !!(entry && typeof entry === "object" && entry.recommended === true);
  }

  function modelEntryTagLabel(entry) {
    // recommended model 的标记文字:优先后端 tag_title(如 "Fast"/"New"),
    // 没有就用 i18n "推荐"。非 recommended 返空(不渲染标记)。
  ...
  function providerModelOptionsMarkup(currentValue = "") {
    // recommended:true 的置顶,其余保持原相对顺序(稳定排序);非推荐仍全量
    // 保留可见,只是排在后面 + 不带标记。其他 provider(全 string entry)
    // recommended 恒 false,排序是 no-op,行为与改前一致。
