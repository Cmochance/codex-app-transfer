...
//   正文 → --font-sans   标题 → --font-serif(国风 h1/h2 用)   等宽 → --font-mono
// 默认值 = tokens.css 既有值(即「国风主题」原本使用的字体):正文=系统 / 标题=宋体 / 等宽=mono。
// 字号 → 按比例缩放全部 --fs-* token。
...
// 各角色:CSS 变量名 + 默认 choice(= 国风原字体)+ localStorage key
const ROLES = {
  body: { varName: '--font-sans', def: 'system' as FontChoice, key: 'cas:fontBody' },
  heading: { varName: '--font-serif', def: 'songti' as FontChoice, key: 'cas:fontHeading' },
  mono: { varName: '--font-mono', def: 'mono' as FontChoice, key: 'cas:fontMono' },
} as const
export type FontRole = keyof typeof ROLES

const SIZE_KEY = 'cas:fontSize'
const SIZE_SCALE: Record<FontSize, number> = { small: 0.9, normal: 1, large: 1.15 }
...
function applyRole(role: FontRole) {
  ...
  // choice 等于角色默认时清除 override → 回落 tokens 默认值(国风字体)
