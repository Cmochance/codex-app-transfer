...
mod tests {
    ...
    #[test]
    fn startup_auto_apply_writes_direct_config_for_lonr_style_provider_with_empty_default_model() {
        // 重现 2026-05-10 现场:用户 active=lonr (apiFormat=responses, baseUrl=lonr.zeabur.app/v1,
        // apiKey=CMC22339, models.default=""), 应用启动 auto_apply_on_startup_if_enabled
        // **应**写出:
        // - ~/.codex/config.toml: openai_base_url = "https://lonr.zeabur.app/v1"
        // - ~/.codex/auth.json: auth_mode="apikey", OPENAI_API_KEY="CMC22339"
        //
        // 用户实际机器上发现 active=lonr 但 codex 配置没 base_url + auth=chatgpt — 该测试
        // 必须能复现这种 silent failure(否则证明 startup 路径 OK,bug 在别处)
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .build()
            .unwrap();
        ...
        with_isolated_home(|home| {
            runtime.block_on(async {
                let mut cfg = config_with_secret();
                cfg["settings"]["proxyPort"] = json!(0);
                cfg["activeProvider"] = json!("lonr-id");
                cfg["providers"] = json!([{
                    "id": "lonr-id",
                    "name": "lonr",
                    "baseUrl": "https://lonr.zeabur.app/v1",
                    "authScheme": "bearer",
                    "apiFormat": "responses",
                    "apiKey": "CMC22339",
                    "models": {
