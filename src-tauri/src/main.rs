    ...
    let app = builder
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                {
                    // MOC-256:无 Chrome 的新装把未设置的 webFetchBackend 持久化为 off(防默认
                    // auto 运行时静默下载 86MB)。helper 与 `--mcp-serve-webfetch` 入口共用,
                    // 覆盖两条进程路径。已设置 / 有 Chrome 则 no-op。
                    crate::admin::services::mcp_servers::default_web_fetch_off_if_no_chrome().await;
