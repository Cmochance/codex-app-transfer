...
**chat 路径 preamble fallback 注入 — 修 Codex 26.609 工具轮间全折叠 (MOC-219)**:Codex Desktop 26.609 把完成态 reasoning(思考块)从对话流渲染中产品级移除(不分 provider、无设置可恢复),工具轮之间唯一持久可见的文本通道变成 assistant message。第三方 chat 模型(MiMo 等)连续工具轮常不吐 message → 在 26.609 下 UI 全部折叠成一条 "Ran N commands" 摘要、且 26.608 时代可见的思考块彻底消失。修复范围:仅 chat 路径 `responses/converter.rs`(gemini/anthropic 路径不动)。① **message 缓冲**:文本 delta 在首个 tool_call 首帧(或流尾)前缓冲不出 wire,判定完成后回到直通流式。这意味着**纯文本回答不再逐字流式,整段在生成完后一次性出现**——这是实现同族折叠 / 异族注入的必要 trade-off,用户会感受到"文字不逐字出现"。② **跨轮工具记忆**(新模块 `responses/preamble.rs`):response_id → 本轮工具族,下一轮按 previous_response_id recall;stateless 客户端 fallback 扫 input 尾部工具段推族。③ **判定**:同族连续工具轮 → 丢弃缓冲(UI 折叠成组,同族内模型偶发的碎 message 一并丢弃、不显示);异族边界 → flush 模型自己的 preamble,没有则截取 reasoning(≤ 300 字符)注入为合成 assistant message——**注入的思考转述会进会话历史 / compact / 切换到真 GPT 时上发**。④ **工具族表**:exec/shell/run 一族、web_search+web_fetch 一族、tool_search 一族、apply_patch 一族、其他按 name 自成一族。Refs MOC-219。
