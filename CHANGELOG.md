...
**gemini/antigravity 会话切真 GPT 报 `call_id: string too long` 修复 (MOC-218)**:gemini 系 provider(Gemini API key / gemini-cli OAuth / Antigravity)会话用过工具后切真 GPT 发消息,composer 内联 400 `Invalid 'input[N].call_id': string too long`(OpenAI Responses 后端上限 64,真机实测被撑到 192~8116 字符)。根因:Gemini 3 functionCall 带 `thoughtSignature` 需后续轮回传,transfer 为无状态往返把签名编码进 call_id(`call_X~~sig~~<sig>`,LiteLLM 模式)并随 `function_call`/`function_call_output` 持久化进 Codex 会话历史;切真 GPT 直连 chatgpt.com 不经 proxy,历史原样上发被后端校验拒。修复:签名出 wire ——`emit_function_call` 改发短 call_id、签名存进共享 `ToolCallCache`(`tool_call_cache.json`,新增 `thought_signature` 字段,旧 JSON 向后兼容),请求侧 `decode_tool_call_id_signature` 三级回退:旧内联格式优先拆(修复前旧会话在 gemini 内续聊签名链不断)→ 本地 cache 反查 → miss 裸发(上游容忍度实测:flash + pro 两档对缺签名历史均 200 正常,代价仅推理连续性)。修复前产生的旧会话历史无法清洗,切真 GPT 仍 400,需开新会话(README FAQ 已注)。Refs MOC-218。
