...
**Codex v26.609 工具轮间思考不显示 — preamble fallback 注入 (MOC-219)**:Codex Desktop v26.609 起不再渲染工具轮之间的 reasoning 思考块,而第三方模型(MiMo 等 chat 系)在连续工具循环中常省略 preamble message → UI 把所有 tool 折叠成一团、轮间完全无文本。不再追逐 renderer 内部渲染条件(26.608→26.609 两连击证明该通道每次 Codex 更新都可能变),改走渲染最稳定的 assistant message 通道:chat 转换路径在 emit function_call 前判定——本轮模型没吐 message 且即将调用的工具是「自上次可见 assistant 文本以来的新工具种类」(从请求历史尾部连续工具段无状态推断)→ 把本轮 reasoning 文本复制为一条合成 message item 注入(`[reasoning, message, function_call]` 标准 wire 序;短的全取,长的按段落累积 ≤300 字符截取);同类工具连续调用不注入,UI 自动折叠成组。合成 message 会进会话历史 / compact / 切真 GPT 上发(assistant message 无后端校验风险)。仅 chat 路径;gemini 系模型 preamble 习惯勤、不需要。Refs MOC-219。
