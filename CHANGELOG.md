...
**回退 chat 路径 preamble fallback 注入(MOC-219)**:Codex Desktop **26.609.41114** 已自行恢复工具轮间 reasoning 思考块渲染,v2.3.2 为应对 26.609.30741 渲染移除引入的 preamble 注入(PR #458)随之变冗余 —— 注入的转述与恢复后的 reasoning 块内容重复、合成 message 污染会话历史、且遗留「每轮注入」问题未解。本次回退 PR #458 全部改动(删除 `responses/preamble.rs`,`converter.rs` 的 message 缓冲 / 段机制还原为纯直通流式),保留其后 MOC-88 等无关改动。/ **Revert chat-path preamble fallback injection (MOC-219)**: Codex Desktop **26.609.41114** restored reasoning-block rendering between tool rounds on its own, making the v2.3.2 preamble injection (PR #458, a workaround for 26.609.30741's render removal) redundant — paraphrases duplicated the restored reasoning blocks, synthetic messages polluted session history, and the per-round injection issue was never resolved. This reverts all of PR #458 (removes `responses/preamble.rs`, restores `converter.rs` to plain pass-through streaming) while keeping later unrelated changes such as MOC-88.

**Codex Desktop pinned summary 用量面板(MOC-204)**:在 Codex 顶栏「Toggle pinned summary」弹窗底部注入可折叠「Usage」分区,展示最多 4 行:① **5 小时额度 / 每周额度**:仅白名单 provider(antigravity gemini 系)显示真实双窗口额度(数据来自 `cloudcode-pa.googleapis.com/v1internal:retrieveUserQuotaSummary`,显**剩余**额度:剩余%=remainingFraction×100,满额=100、≤10% 红色预警 + 重置时刻);其余 provider 不显额度行。② **上下文**:注入脚本从 Codex React fiber 直读 `usedTokens`/`contextWindow`——有历史对话立即有值(无需新对话);满窗口=contextWindow÷0.95(加回 Codex 5% reserve);1M 模型显「1M」而非「1000k」。③ **Tokens 实时速率·累计**:速率由 MutationObserver 监测 Codex 流式文本增量估算(2s 滑窗、CJK 感知,流停冻结最后值);累计来自 Codex rollout 文件(当前会话 ground truth)。④ **缓存命中率**:来自 rollout cached_input/input。「Usage」标题可折叠,状态 localStorage 持久。注入走 CDP 周期推送(5s tick),页面刷新 / 重启后自动重挂。设置 → 「Codex 内显示用量信息」(默认关);需本应用启动 Codex(共用 CDP 通道),Codex 已在运行时需重启生效。/ **Codex Desktop pinned-summary usage panel (MOC-204)**: injects a collapsible "Usage" section at the bottom of Codex's "Toggle pinned summary" popup (alongside Environment / Sources), showing up to 4 rows: ① **5-hour / weekly quota** — whitelisted providers only (antigravity gemini series); real dual-window data from `cloudcode-pa.googleapis.com/v1internal:retrieveUserQuotaSummary` (**remaining** quota: remaining% = remainingFraction×100, full = 100, red warning ≤10% + reset time); other providers produce no quota rows. ② **Context** — injected JS reads `usedTokens`/`contextWindow` directly from Codex's React fiber, immediately available for any existing conversation; full window = contextWindow ÷ 0.95 (restores the 5% reserve Codex hides); 1M models show "1M" not "1000k". ③ **Tokens real-time rate · cumulative** — rate estimated via MutationObserver on Codex's streaming text (2s sliding window, CJK-aware, freezes last value when stream stops); cumulative total from Codex rollout (current session). ④ **Cache hit rate** — from rollout cached_input/input. "Usage" header is collapsible (localStorage-persisted). CDP periodic push (5s tick); re-attaches after page reload / restart. Settings → "Show usage in Codex" (off by default); requires Codex launched from this app; restart Codex after toggling if already running.

**Usage 面板按活动对话隔离(MOC-230)**:MOC-204 面板的累计 token / 缓存命中率 / 实时速率从「最近修改的 rollout」改为按**活动对话**精确取数 —— 注入脚本从 React fiber 读当前 `conversationId`(== rollout 文件名 uuid == session_meta payload.id,解包 + 真机实证),daemon 经 `Runtime.evaluate` 回读后按该 id 取对应 rollout(`session_totals_for_id`,非 newest-mtime);多对话切换不再串号(切回旧对话即显示该对话自己的累计/缓存,速率清零重估)。读不到 id / 无对应 rollout 显「—」(fail-closed,绝不显示别的对话数据);JS 侧再加 convId-match 守卫双保险。INSTALL_SCRIPT 加版本化幂等 guard,应用升级后无需重启 Codex 即覆盖旧注入。/ **Per-conversation isolation for the Usage panel (MOC-230)**: the MOC-204 panel's cumulative tokens / cache-hit / live rate now key to the **active conversation** instead of the most-recently-modified rollout — injected JS reads the current `conversationId` from the React fiber (== rollout filename uuid == session_meta payload.id), the daemon reads it back via `Runtime.evaluate` and loads that conversation's rollout (`session_totals_for_id`, not newest-mtime); switching conversations no longer cross-contaminates (switching back to an older conversation shows its own cumulative/cache; the rate resets). Shows "—" (fail-closed, never another conversation's data) when the id / rollout can't be resolved, with a JS-side convId-match guard as a second layer. INSTALL_SCRIPT gains a versioned idempotency guard so app upgrades override the old injection without a Codex restart.
...
**Codex 主题对话区可读性优化 (MOC-225)**:主题注入的对话区背景此前几乎透明、亮壁纸下文字看不清 —— `#root` 只有单层 scrim(中部仅 ~0.34),且 11 套主题 scrim 强度统一未按壁纸亮度区分。本次把 `#root` 升级为 **3 层复合 scrim**(顶部阻尼带 + 居中对话列焦点径向加强 + 加深基线,`color-mix` 复用最深 scrim 色),并按各套壁纸焦点亮度逐套校准 8 套亮壁纸的 scrim 强度(`azurlane` / `zani` / `frost` / `nailin` / `nocturne` / `duet` / `sonata` / `rose`;暗壁纸 `carton` / `changli` / `studio` 保持)。同配色下中央有效遮罩从 ~0.34 提到 ~0.6+,文字清晰且不糊死壁纸。方案借鉴同源的 agent-theme 已优化版。Refs MOC-225。

**chat 路径 preamble fallback 注入 — 修 Codex 26.609 工具轮间全折叠 (MOC-219)**:Codex Desktop 26.609 把完成态 reasoning(思考块)从对话流渲染中产品级移除(不分 provider、无设置可恢复),工具轮之间唯一持久可见的文本通道变成 assistant message。第三方 chat 模型(MiMo 等)连续工具轮常不吐 message → 在 26.609 下 UI 全部折叠成一条 "Ran N commands" 摘要、且 26.608 时代可见的思考块彻底消失。修复范围:仅 chat 路径 `responses/converter.rs`(gemini/anthropic 路径不动)。① **message 缓冲**:文本 delta 在首个 tool_call 首帧(或流尾)前缓冲不出 wire,判定完成后回到直通流式。这意味着**纯文本回答不再逐字流式,整段在生成完后一次性出现**——这是注入判定的必要 trade-off,用户会感受到"文字不逐字出现"。② **模型 message 永远原样下发**:模型自己吐的 preamble 一字不动展示(工具折叠是 Codex 渲染层自己的职责,代理不替模型删话)。③ **轮数节流注入**(新模块 `responses/preamble.rs`):跨轮记忆「自上次可见文本以来的连续静默工具轮数」(response_id → 计数,按 previous_response_id recall;stateless 客户端 fallback 数 input 尾部工具段),模型静默的工具轮连续满 3 轮(或无记录的新会话首轮)才注入一句 reasoning 转述(≤ 300 字符)为合成 assistant message——UI 严格保持「最多每 3 个静默工具轮一句话」,既不全程空白也不每轮刷屏;**注入的思考转述会进会话历史 / compact / 切换到真 GPT 时上发**。Refs MOC-219。

**Antigravity 原生图像生成(image_gen)接入 (MOC-210)**:Codex 内置的 `image_gen` 图像生成工具在 antigravity provider 上现在真正出图(原生,非 CLI fallback)。此前 `image_generation` 工具对所有 provider 默认 drop(模型收不到 → imagegen 技能报"工具不可用")。本次链路:① 请求侧把工具降级成 `image_gen` functionDeclaration、**仅对 antigravity 暴露**(其余 provider 无履约端,暴露会导致 dangling function_call 卡 Codex);② 模型在对话里调 `image_gen` → gemini 回 `functionCall(image_gen)`,代理在响应流里**逐事件透传**文本/思考、检出 image_gen 调用时截获 → 发出图子请求(默认 `gemini-3.1-flash-image`,可经 provider 配置页 `gpt-image-1` / `image` 槽位覆盖)→ 把返回的 `inlineData` 图片转成 OpenAI Responses `image_generation_call`(`result` 裸 base64,对齐 codex-rs)内联回流 → Codex 渲染并落盘;③ 出图轮在 session 历史留痕,避免模型误判未出图而重复调用。仅 antigravity 路径。Refs MOC-210。

**antigravity gemini 全系 context window 兜底 258k → 1M (MOC-221)**:antigravity 的 gemini 模型在 Codex 里 context window 此前显示 258k(项目兜底默认 `DEFAULT_CONTEXT_WINDOW=258_400`),但上游对全系 gemini 文本模型声明 `context_length=1_048_576`(1M)。根因:`documented_context_window` 只精确匹配 `gemini-3-flash` / `gemini-2.5-flash` / `gemini-3.1-flash-lite` 三个 id,真机实际映射到 Codex 槽位的带后缀 id(`gemini-3-flash-agent` / `gemini-3.1-pro-low` / `gemini-pro-agent` 等)全不命中 → 落 258_400 兜底,导致 auto-compact 在 ~207k(258400×80%)就过早触发,只用到 gemini 1M 的约 20% 上下文。改为前缀匹配(`gemini-*` 文本模型统一 1M,`gemini-*-image` 图像模型单独排除留给后续接入),catalog 写 1_000_000、auto-compact 阈值提到 800k、Codex 上下文圆环显示对齐真实 1M。纯 documented 默认值修正,不动协议 / wire 指纹。Refs MOC-221。

**用量统计打磨 — 时区 / 精度 / 性能 (MOC-19)**:承接 token 用量统计的 4 项 followup。①「最后活动」时间改为后端按用户时区显示(此前是 UTC,跟「按日」视图的本地日期不一致,体感时间对不上);② 取每对话「最后活动」从字符串比较改为按真实时刻(epoch 毫秒)比较,避免混合时区偏移(`+08:00` vs `Z`)的 RFC3339 字符串序 ≠ 时间序导致选错最晚活动;③ `/api/usage/summary`(全扫 ~1.2GB rollout、release 约 1-2s)加 60 秒 TTL 缓存 + single-flight,多标签页 / 重复打开 Usage 页不再各自冷扫,**用户主动点「刷新」仍强制拿最新**(走 nocache 绕过缓存);④ rollout 文件打开失败从静默跳过改为 tracing 告警(用户报「数据少了几天」时可在日志定位)。Refs MOC-19。

**第三方会话切真 GPT 两连 400 修复:`call_id: string too long` + `reasoning content: array too long` (MOC-218)**:OpenAI Responses 后端对上发历史逐项校验,transfer 会话历史里有两类不合规 wire 痕迹,切真 GPT(直连 chatgpt.com 不经 proxy,无法清洗)时依次被拒。① **call_id 超长**(gemini 系专属):Gemini 3 functionCall 带 `thoughtSignature` 需后续轮回传,transfer 为无状态往返把签名编码进 call_id(`call_X~~sig~~<sig>`,实测 192~8116 字符,后端上限 64)。修复:签名出 wire——`emit_function_call` 改发短 call_id、签名存进共享 `ToolCallCache`(`tool_call_cache.json` 新增 `thought_signature` 字段,旧 JSON 向后兼容),请求侧三级回退:旧内联格式优先拆(修复前旧会话在 gemini 内续聊签名链不断)→ 本地 cache 反查 → miss 裸发(上游容忍度实测:flash + pro 两档对缺签名历史均 200,代价仅推理连续性)。② **reasoning item 带明文 `content`**(gemini + chat 双路径,v2.3.1 MOC-203 双通道引入):后端硬校验 input 里 reasoning item 的 `content` 数组长度必须 0(`array_above_max_length`,真 GPT 自家 rollout 的 reasoning item 即无 content 字段),`encrypted_content: null` 假值另有 MOC-13 `invalid_encrypted_content` 前科。修复:reasoning **item**(added/done/envelope)不再带 `content` / `encrypted_content` 字段,出生即合规;思考的当轮渲染不受影响——新版(v26.608+)读 SSE `reasoning_text.delta` content 通道**事件**(保留双发),实证 GPT 直连 `summary:[]` 仍正常显示即渲染靠事件流不靠 item 字段;item.summary 保留非空供旧版兼容。修复前产生的旧会话历史无法清洗,切真 GPT 仍 400,需开新会话(README FAQ 已注)。Refs MOC-218。
...
**MCP 工具图标 (MOC-190 followup)**:cat-webfetch 的 `serverInfo.icons` 用本应用图标(MCP 2025-11-25 SEP-973);Codex 折叠的「工具活动」汇总会渲染它。注:展开的单条工具调用 header 图标由 Codex 的 connector catalog 控制、不读 MCP icons,故只影响折叠汇总。Refs MOC-190。
...
**remote compaction v2 支持 (MOC-198)**:新版 Codex(启用 `remote_compaction_v2`)autocompact 不再调私有 `/responses/compact` 端点,改发普通流式 `/responses` 请求并在 `input` 末尾追加 `{"type":"compaction_trigger"}` 标记;此前代理未识别该形态——把它当普通对话处理,响应中不含 compaction item,Codex 报 `remote compaction v2 expected exactly one compaction output item` 后挂住。修复:新增 `detect_compact` 双轨检测(V1 按 path;V2 按「普通 /responses + body 含 compaction_trigger」两段扫,廉价字节筛后 parse 确认);V2 路径剥掉标记 item 后复用现有摘要生成逻辑,响应按 Codex v2 协议包装成 SSE 流(`response.created` → `response.output_item.done` → `response.completed`,单 compaction item);失败路径 emit `response.failed`(HTTP 仍 200,对齐 MOC-103 非裸错误约定)。四大 mapper(chat / gemini_native / cloud_code / anthropic)均已适配;grok_web 与 responses 透传 provider 例外(grok 上游本就不支持 compact、与 V1 现状平价;透传系交由上游原生处理,同 MOC-113 约定)。旧版 Codex 仍走 V1 端点,两路并存不互斥。
