...
- 重写 chat-path V4A 教学 prompt:单端 `@@ <header>` 语法、minimal Update form、Add File 全 `+` 前缀、`*** Begin Patch` literal 第一行、Update + Move 必须配 ≥1 hunk(纯重命名用 Delete + Add 替代)。
- `response.completed` envelope `output[]` 终态跟流式 `response.output_item.done` 一致,interrupted `apply_patch` 两侧都 `incomplete`,严格客户端读 envelope 不再误执行 partial patch。
...
- Rewrote the chat-path V4A teaching prompt: single-sided `@@ <header>` syntax, minimal Update form, Add File requires `+` on every line, `*** Begin Patch` must be the literal first line, Update + Move requires at least one hunk (use Delete + Add for pure rename).
- The `response.completed` envelope `output[]` final state matches the streaming `response.output_item.done` event; interrupted `apply_patch` is `incomplete` on both sides, so strict clients reading the envelope no longer execute partial patches.
