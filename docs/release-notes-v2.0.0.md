...
- **部分 v1.4 路由先 stub**:测速 / 自动探测可用模型 / autofill / 兼容性矩阵 / 反馈上送 / config import 在 v2.0.0 首发时返成功但不实际工作,留 v2.0.x 接通。后续版本已经陆续恢复这些缺口,当前主线状态以 README 和 `v1.0.3-gap-task-plan.md` 为准。
...
- **Some v1.4 routes were stubs at v2.0.0 launch**: speed test / available-model probing / autofill / compatibility matrix / feedback submission / config import returned success but no-op in the initial v2.0.0 release. Later v2.0.x work has been restoring these gaps; use the README and `v1.0.3-gap-task-plan.md` for the current main-branch status.
