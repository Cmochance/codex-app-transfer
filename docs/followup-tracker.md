...
- ~~#34 客户端 latest.json + installer RSA 验签~~ → PR #197 (2026-05-17),signature.rs verify_signed_bytes + update.rs:521 双 gate(先验签后 sha256),MITM 改 sha256 攻击路径已堵
- ~~#37 update.rs download_asset_impl TOCTOU + bad-sha test~~ → PR #199 (2026-05-17),in_memory Vec<u8> 流式接收 + 验签 sha256 通过才落盘消除中间文件窗口,bad-sha mismatch 单测 re-add 覆盖 4 路径
