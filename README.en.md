...
- **Codex Desktop pet focus-steal suppression (on by default)**: When Codex Desktop is launched from this app, transfer injects an anti-focus patch into the `avatar-overlay` CDP page to stop the desktop pet from forcing Codex back to the foreground where possible. The workaround can be disabled in Settings; disabling stops future automatic injection and does not patch Codex Desktop `.asar` or system settings
