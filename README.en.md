...
Current version **v2.3.1** (see [Changelog](CHANGELOG.md) and [Releases](https://github.com/Cmochance/codex-app-transfer/releases)).
...
- Reasoning (thinking) blocks now display correctly in Codex Desktop v26.608+: the new renderer reads the reasoning **content channel** (`reasoning_text` + `content_index`) instead of the old summary channel; gemini_native and chat paths now dual-emit both channels for compatibility with both old and new Codex versions. Chat path also fixes interleaving of reasoning and tool_call stream events (reasoning is closed before opening a new tool item); gemini path fixes tool-call grouping (`functionCall`-following empty text parts no longer produce a blank message item, so same-turn tools fold correctly) (MOC-203). **Codex 26.609+ preamble fallback injection (MOC-219)**: 26.609 removed completed reasoning blocks from the render layer at the product level (applies to all providers, no setting to restore it), making the assistant message the only persistent visible text channel between tool rounds — third-party models that produce no message during consecutive tool rounds now show an entirely collapsed "Ran N commands" summary in 26.609+. The chat path adds preamble fallback: consecutive same-family tool rounds (exec/shell family, web_search+web_fetch family, etc.) are folded into a group; at a family boundary the model's own preamble is flushed, or a reasoning excerpt (≤ 300 chars) is injected as a synthetic assistant message, matching the natural "one preamble between tool groups" behavior of the gemini family. **Note**: ① Plain-text replies no longer stream word-by-word — the full reply appears at once after generation completes (buffering trade-off: the proxy must wait for the first tool_call frame before it can decide whether to fold); ② Incidental short messages from the model during a same-family tool run are folded and discarded (not shown, not saved to history); ③ Injected reasoning excerpts are written into conversation history / compact / and uploaded when switching to real GPT
