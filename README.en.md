...
- Reasoning (thinking) blocks now display correctly in Codex Desktop v26.608+: the new renderer reads the reasoning **content channel** (`reasoning_text` + `content_index`) instead of the old summary channel; gemini_native and chat paths now dual-emit both channels for compatibility with both old and new Codex versions. Chat path also fixes interleaving of reasoning and tool_call stream events (reasoning is closed before opening a new tool item); gemini path fixes tool-call grouping (`functionCall`-following empty text parts no longer produce a blank message item, so same-turn tools fold correctly) (MOC-203). **Codex 26.609+ preamble fallback injection (MOC-219)**: 26.609 removed completed reasoning blocks from the render layer at the product level (applies to all providers, no setting to restore it), making the assistant message the only persistent visible text channel between tool rounds — third-party models that produce no message during consecutive tool rounds now show an entirely collapsed "Ran N commands" summary in 26.609+. The chat path adds preamble fallback: the model's own preamble messages are always shown verbatim; for silent tool rounds a reasoning excerpt (≤ 300 chars) is injected as a synthetic assistant message under a round-count throttle — only after 3 consecutive silent rounds (or on the first round of a fresh session with no memory), keeping the UI at "at most one line per 3 silent tool rounds", neither fully blank nor spamming every round. **Note**: ① Plain-text replies no longer stream word-by-word — the full reply appears at once after generation completes (buffering trade-off: the proxy must wait for the first tool_call frame before the injection decision); ② Injected reasoning excerpts are written into conversation history / compact / and uploaded when switching to real GPT
...
- **apply_patch middle layer (format recovery)**: third-party chat models lack GPT's lark-grammar-constrained generation, so they often emit malformed V4A (double-sided `@@`, missing `+` on Add File lines, byte-mismatched context, missing `*** Begin/End Patch` envelope, dropped blank lines, missing line prefixes, a redundant `@@`/`-` anchor pair, a leaked envelope marker at the end of an Add File body, etc.). The middle layer recovers each known error to valid format before sending to Codex — reading the file from disk to align `@@` anchors / context to real bytes, restoring dropped blank lines, converting empty-file / rename-only into `Delete+Add`, de-duplicating a redundant `@@` anchor when `@@ X` is immediately followed by an identical `-X` (glm-5.1 observed: the model used one line as both the `@@` locator and the `-` deletion; only inside `*** Update File:` sections), stripping a leaked `+*** End Patch` envelope marker from the end of an Add File body (glm-5.1 observed: the model treated the envelope as file content, prefixed it with `+`, and wrote it into the file), etc.; **non-destructive** (never loses content or overwrites) and **passes unknown cases through untouched** (let Codex error so the model self-corrects, never guesses). It mirrors the V4A lark grammar Codex constrains GPT with, enforced post-hoc on the chat path (credit to [openai/codex](https://github.com/openai/codex)'s apply_patch lark grammar) (MOC-194 / MOC-228)
