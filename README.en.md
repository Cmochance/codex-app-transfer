...
### After a Codex update, consecutive third-party tool calls "collapse into one group with no thinking text in between"

Codex Desktop v26.609 stopped rendering reasoning blocks between tool rounds; third-party models (MiMo etc.) often skip the preamble message during continuous tool loops, so the UI collapses all tool calls into a single group with no visible text in between. **Adapted (MOC-219)**: on the chat path, when a tool kind appears for the first time and the model said nothing that round, transfer copies that round's reasoning text (≤300 chars) into a synthesized assistant message, restoring visible narration between tool groups; consecutive calls of the same tool kind are not injected and still fold automatically. Note: the injected text enters the conversation context (visible to the model) — this is expected behavior.
