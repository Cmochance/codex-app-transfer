...
| Grok Build (xAI grok CLI coding backend, `cli-chat-proxy.grok.com/v1`, responses) | 🧪 | 🧪 | 🧪 | **Code-complete; login + end-to-end pending real-machine verification (MOC-299)**. Native responses passthrough (reuses `ResponsesPassthroughMapper`, no new adapter/mapper); account login via OAuth2 device flow (`authScheme=grok_build_oauth`, click login → browser auth at `accounts.x.ai/oauth2/{device/code,token}` + auto refresh, credential stored in `grok-build-oauth.json`, **no local grok CLI install required**); outbound injects grok-shell client fingerprint headers (UA / x-xai-token-auth / x-grok-client-* / x-grok-model-override, see forward.rs `injects_grok_build_headers`) to avoid risk control. **Model mapping**: Codex's gpt-5.x names are mapped to grok models via `provider.models` slots, never passed through to the upstream (which wouldn't recognize them). Models: grok-build (512k, coding) / grok-composer-2.5-fast (200k). TOS gray, hidden by default, personal use only |
