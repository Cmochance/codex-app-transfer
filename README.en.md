...
- **Synthetic account Plugins unlock** (MOC-257): a force-unlock tier for when there's no real ChatGPT account, replacing the old CDP injection (which only changed the renderer UI while `auth.json` stayed apikey → an inconsistent transport where plugins didn't actually work). Writes a well-formed synthetic `auth.json` (`auth_mode=chatgpt` + a synthetic JWT with a far-future `exp` to keep the CLI from refreshing, plus a `cas_synthetic` sentinel) so Codex shows Plugins **natively**; while synthetic mode is on, the proxy **fabricates 200 responses** for the `/backend-api/*` account/plugin requests Codex sends (**never 401** — a 401 would trigger Codex re-login) instead of passing them through to the real chatgpt.com. Conversations still go through your third-party provider (the resolver only validates the JWT shape, not the signature). **The marketplace is empty for now** — only locally-installed plugins under `~/.codex/plugins/cache/` (MCP servers / skills that run locally) are shown; no cloud marketplace browsing / install. A "Unlock Plugins via synthetic account" Settings switch, **mutually exclusive with real-account mode**; it refuses to enable (and won't overwrite a real account) when a ChatGPT login state is present (including tokens preserved after switching to apikey)
