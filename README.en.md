...
- **Wire-level dedup of repeated system blocks (MOC-193)**: Codex sends a full env block (~37 KB) with every turn; long conversations accumulate multiple identical system/developer instruction messages. The chat path automatically deduplicates identical blocks after history merge and before forwarding upstream (keeping the first occurrence to preserve the prompt-cache prefix), saving tens of KB per turn in long sessions; the session cache is kept intact
