...
- **Active-conversation top strip inside Codex Desktop (optional, off by default)**: Settings → "Show active conversations at the top of Codex" injects a top strip in the main Codex view showing active conversation titles and status. Each item supports click-to-switch and a direct close/archive action, reducing repeated left-sidebar scrolling when juggling many project conversations. Injection uses periodic CDP pushes and re-attaches after a Codex reload/restart; requires launching Codex through this app.
