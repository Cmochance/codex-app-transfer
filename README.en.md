...
- **Plugin marketplace + Plugins enhancements (MOC-7)**: browse a mirror of OpenAI Codex's plugin directory by category under `Docs → MCP → Marketplace` (178 connectors + search + icons + website links); data comes from a project-maintained private storage repo (`codex-app-transfer-storage`, the backend holds a read-only token to fetch the registry + proxy icons + cache); **multi-source aggregation** — an official source plus your own public connector registries via "Add source" (same `{connectors}` schema). **Browse-only**: these are OpenAI-platform OAuth remote connectors brokered by a single `plugin-runtime` MCP server (`chatgpt.com/backend-api/ps/mcp`) with no standalone MCP endpoint, so the mirror holds display data only. Also: `Docs → MCP → Plugins` now shows an icon (`assets/app-icon.png`) per installed plugin + a "Skills" button (popup shows its `SKILL.md`; a dropdown picks one when a plugin has multiple skills)
