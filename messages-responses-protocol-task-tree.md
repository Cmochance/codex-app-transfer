...
> 当前状态: P16 已通过 Anyrouter forced tool-call + `function_call_output` continuation 代理端到端验证;第一轮返回 `function_call`,第二轮 `previous_response_id` 续轮返回 `done` 且 `response.completed.status=completed`。
...
P16 已完成请求构造层修复和 Anyrouter 代理端到端验证。下一步可继续扩大真实 provider 覆盖:structured output、MCP/custom tool metadata、code execution、upstream error 与长上下文续轮。
