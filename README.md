...
- **Codex Desktop 宠物防抢焦(默认开)**:通过本应用启动 Codex Desktop 时,transfer 会给 `avatar-overlay` CDP page 注入 anti-focus patch,尽量阻止桌面宠物把 Codex 窗口强制拉回前台。设置页可关闭该 workaround;关闭后不再自动注入,不改 Codex Desktop `.asar` / 系统配置
