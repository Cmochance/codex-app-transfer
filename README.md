...
当前版本 **v2.4.3**(详见 [Changelog](CHANGELOG.md) 与 [Releases](https://github.com/Cmochance/codex-app-transfer/releases))。
...
- **任务完成后过程自动折叠(对齐官方 GPT 行为,MOC-293)**:走 Chat Completions 转换的 provider(如 GLM / Kimi / DeepSeek / WorkBuddy)整轮处理过程(思考 + 工具调用 + 铺垫消息)在最终答复出来后自动折叠成 "Worked for Ns",只展开最终回复——与官方 ChatGPT 账号一致。实现:chat 转换路径给 assistant message item 补官方 wire 的顶层 `phase` 字段(工具轮铺垫 → `commentary` 归入折叠过程区,最终答复 → `final_answer` 展开);该路径 message 只在流末闭合,收尾 done 时工具调用已全部可见、phase 天然权威,流式 open 事件先发临时 `commentary`,保证折叠只在真最终答复收尾时触发一次、任务中途不抖动。注:最终答复流式期间先显示在过程区、收尾时提升为最终回复(第三方模型无法像 GPT 那样预先声明消息通道)。**暂不覆盖**:gemini_native(Antigravity)与 anthropic_messages(anyrouter)转换路径因流内块序会在 message 闭合后才出现工具调用、close 时无法可靠判定终态(误发会导致每轮工具误折叠再展开),暂不补 phase(留 followup);grok_web 与 Responses 直通不经此转换、不涉及
