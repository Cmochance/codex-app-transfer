...
- 当前版本:**v2.0.12**(Python → Rust/Tauri 全栈重写后的当前主线;UI 视觉与 v1.0.4 字节级一致)
...
- `Codex-App-Transfer-v<版本>-macOS-x64.dmg`：macOS Intel x64 拖拽安装(v2.0.13+ 提供,见 [issue #61](https://github.com/Cmochance/codex-app-transfer/issues/61))
...
- Current version: **v2.0.12** (current mainline after the full Python → Rust/Tauri rewrite; UI byte-identical to v1.0.4)
...
- Platforms: v2.0.0 launched on macOS arm64 only; v2.0.1+ release builds produce macOS arm64 / Windows x64 / Linux x86_64 assets; **v2.0.13+ also ships macOS Intel x64** (see [issue #61](https://github.com/Cmochance/codex-app-transfer/issues/61)). v2.0.0 ~ v2.0.5 are archived as intermediate releases (source-only, no binaries — see each Release page); use v2.0.12 in production. Earlier Python v1.0.x line is no longer recommended for new installs — go straight to v2.0.x.
