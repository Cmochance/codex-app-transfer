...
- **插件市场(展示镜像,MOC-7)**:`文档 → MCP → Marketplace` 子区(官方源)浏览 OpenAI Codex 插件目录的镜像(178 个连接器,按分类 + 搜索 + 图标 + 官网链接),数据来自项目维护的私有 storage 仓库(`codex-app-transfer-storage`),后端持只读 token 代拉 registry + 图标代理 + 缓存(内存 30min / 图标磁盘)。**仅展示浏览**——这些连接器是 OpenAI 平台的 OAuth 远程连接器、由单一 `plugin-runtime` MCP server(`chatgpt.com/backend-api/ps/mcp`)统一 broker、无独立 MCP 端点,故镜像目录只含展示数据(名称 / 分类 / 描述 / 图标 / 官网),不含可连接配置
