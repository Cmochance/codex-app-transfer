...
- **模拟账号 Plugins 解锁**(MOC-257):无真实 ChatGPT 账号时的强制解锁档,替代旧 CDP 注入(只改渲染层、`auth.json` 仍 apikey → 通信不自洽、插件跑不通)。写一份合规伪造的 `auth.json`(`auth_mode=chatgpt` + 合成 JWT、远未来 `exp` 防 CLI 刷新、`cas_synthetic` 哨兵)让 Codex **原生**显示 Plugins;proxy 在伪造模式开时把 Codex 发的 `/backend-api/*` 账号 / 插件请求**逐条伪造 200**(**绝不 401** —— 401 会触发 Codex 重登),而非透传真 chatgpt.com。对话照常走第三方 provider(resolver 只验 JWT 形状不验签)。**marketplace 本期为空目录** —— 只显示本地 `~/.codex/plugins/cache/` 已装插件(MCP server / skill 本地执行),不含云端 marketplace 浏览 / 安装。设置页「模拟账号解锁 Plugins」开关,**与真实账号模式互斥**;有 ChatGPT 登录态(含已切 apikey 但保留的 tokens)时拒绝开启、不覆盖真账号
