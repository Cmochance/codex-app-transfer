...
- **Codex 顶部活跃对话条(可选,默认关)**:设置 →「Codex 顶部显示活跃对话」,在主界面顶部注入活跃对话条,显示标题 + 状态。支持点击条目快速切换到对应对话,并可在条目右侧直接关闭/归档该对话,减少跨项目多会话时反复滚动左侧列表的操作。注入走 CDP 周期推送,页面刷新 / 重启后自动重挂;需通过本应用启动 Codex。
