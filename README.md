...
当前版本 **v2.3.1**(详见 [Changelog](CHANGELOG.md) 与 [Releases](https://github.com/Cmochance/codex-app-transfer/releases))。
...
- 思考过程(reasoning)在新版 Codex Desktop(v26.608+)正常显示:新版渲染读 reasoning **content 通道**(`reasoning_text` + `content_index`)而非旧 summary 通道,gemini_native / chat 路径已双发两通道兼容新旧版;chat 路径同时修复 reasoning 与 tool_call 流式事件交错(开工具前先闭合 reasoning)、gemini 修复工具调用折叠(`functionCall` 后空 text part 不再产生空 message item,同轮工具正常折叠成组)(MOC-203)。**Codex 26.609+ preamble fallback 注入(MOC-219)**:26.609 把完成态 reasoning 从渲染层产品级移除(不分 provider、无设置),工具轮间唯一可见文本通道变成 assistant message——第三方模型连续工具轮不吐 message 时 UI 在 26.609 下全折叠成 "Ran N commands" 摘要。chat 路径新增 preamble fallback:同族连续工具轮(exec/shell 同族、web_search+web_fetch 同族等)折叠成组;异族边界注入模型自己的 preamble 或 reasoning 转述(≤ 300 字符),UI 形态对齐 gemini 系「每个工具组间一句 preamble」的自然行为。**注意**:① 纯文本回答不再逐字流式——文字在生成完后整段一次性出现(缓冲 trade-off,必须等首个 tool_call 到才能判定是否折叠);② 同族连续工具轮里模型偶发的碎 message 会被折叠丢弃(不显示、不进历史);③ 注入的思考转述会进会话历史 / compact / 切换到真 GPT 时上发
...
> **MCP 工具(Codex 0.130+ `tool_search` 机制)**:Codex 0.130+ 把 server-side MCP 工具(`mcp__notion__*` / `mcp__linear__*` 等,以及内置 `cat-webfetch` 的 web_fetch / web_search)defer 到 `tool_search`,不再直接放进 `tools[]`。代理在 **chat 路径**与 **Gemini / Antigravity 路径**均已打通全链路 —— 从 `tool_search_output` 发现工具 → 注入下游 `tools[]` / `functionDeclarations` → 按 `namespace` 路由回上游(chat #293;Gemini/Antigravity MOC-217)。**上表所有 chat-compat + Gemini 系 provider 通用**;仅 Responses 协议透传(末行,不经代理)不适用。此前 Gemini / Antigravity 路径漏接 `tool_search`,导致这些 provider 下所有 defer 的 MCP 工具(含内置 web_fetch / web_search)对模型不可见 —— MOC-217 已修复。
