...
当前版本 **v2.4.1**(详见 [Changelog](CHANGELOG.md) 与 [Releases](https://github.com/Cmochance/codex-app-transfer/releases))。
...
| WorkBuddy(腾讯 CodeBuddy,`copilot.tencent.com/v2`,openai_chat) | 🧪 | 🧪 | 🧪 | 复用桌面端登录 token(~1 年期 Keycloak Bearer)直连,纯 bearer 无需额外头;协议走通用 openai_chat adapter(同 DeepSeek/MiniMax);出站自动伪装成桌面端 coding 模式 wire 指纹(X-Agent-Intent: coding + X-IDE-* + OpenAI SDK UA/X-Stainless-* + 每请求 X-Conversation-*,见 forward.rs `injects_workbuddy_source_headers`)避风控;**当前仅冒烟(单轮直连各模型回包正常,含完整指纹头实测服务端接受),多轮/autocompact/tool 修复长期真机回归待补**。实测可用 id:deepseek-v4-pro/flash、glm-5.2/5.1/5v-turbo、minimax-m3、kimi-k2.7/k2.6、hy3-preview、auto;渠道(如中国移动 Token Plan)开通子集不同,Claude(default-1.x)等可能未开通。**两种接入路**:`workbuddy`(粘 token)+ `workbuddy-login`(账号登录 OAuth,`authScheme=workbuddy_oauth`,点登录走 `/v2/plugin/auth/{state,token}` 外链轮询拿凭证 + 自动 refresh,免手抠 token;账号登录端到端需真机验证) |
