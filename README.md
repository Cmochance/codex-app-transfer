...
- **插件市场 + Plugins 增强(MOC-7)**:`文档 → MCP → Marketplace` 子区按分类浏览 OpenAI Codex 插件目录的镜像(178 个连接器 + 搜索 + 图标 + 官网链接),数据来自项目维护的私有 storage 仓库(`codex-app-transfer-storage`,后端持只读 token 代拉 registry + 图标代理 + 缓存);**多源聚合**——官方源 + 「添加源」加自定义公开连接器目录(同 `{connectors}` schema)。**仅展示浏览**:这些连接器是 OpenAI 平台的 OAuth 远程连接器、由单一 `plugin-runtime` MCP server(`chatgpt.com/backend-api/ps/mcp`)统一 broker、无独立 MCP 端点,故镜像目录只含展示数据。另:`文档 → MCP → Plugins` 子页给已安装 plugin 显示图标(`assets/app-icon.png`)+ 「Skills」按钮(弹窗看其 `SKILL.md`,多 skill 下拉选条)
