...
- **任务完成后过程自动折叠(对齐官方 GPT 行为,MOC-293)**:第三方模型的整轮处理过程(思考 + 工具调用 + 铺垫消息)在最终答复出来后自动折叠成 "Worked for Ns",只展开最终回复——与官方 ChatGPT 账号一致。实现:三条转换路径(chat / gemini_native / anthropic_messages)给 assistant message item 补官方 wire 的顶层 `phase` 字段(工具轮铺垫 → `commentary` 归入折叠过程区,最终答复 → `final_answer` 展开);流式 open 事件一律先发 `commentary`、收尾 done 给权威值,保证折叠只在真最终答复收尾时触发一次、任务中途不抖动。注:最终答复流式期间先显示在过程区、收尾时提升为最终回复(第三方模型无法像 GPT 那样预先声明消息通道)
