...
- **多轮重复 system 块 wire 级去重(MOC-193)**:Codex 每轮请求携带完整 env block(~37 KB),长对话累积多份相同的 system/developer 指令块;chat 路径在 merge 历史后、发上游前自动去重相同块(保留首次出现以稳定 prompt-cache prefix),实测可省数十 KB/轮;session cache 保持全量原貌不受影响
