...
- **apply_patch 中间层(格式恢复)**:第三方 chat 模型无 GPT 的 lark 语法约束生成,常产出畸形 V4A(双边 `@@`、Add File 漏 `+`、上下文 byte 失配、缺 `*** Begin/End Patch` 信封、漏空行、漏前缀、**多个不连续 hunk 漏 `@@` 分隔**等)。中间层按**白名单逐条恢复**成合法格式再发 Codex —— 读盘把 `@@` 锚点 / 上下文对齐成文件真实字节、补回漏写空行、空文件 / 空 rename 转 `Delete+Add`、**多区编辑漏 `@@` 时按文件真实位置自动切段插裸 `@@`**(MOC-263 P0:仅唯一可分段才动,浮动 `+` 落点歧义则透过)等;**非破坏性**(不丢内容、不覆盖)、**未知一律原样透过**(交 Codex 报错让模型自纠,不猜)。读盘解析的 cwd 用**最近若干 cwd 候选**而非单一全局值,使并发多会话下兜底不再被别项目的 cwd 覆盖失效(MOC-263 P1)。思路镜像 Codex 给 GPT 用的 V4A lark 语法约束、在 chat 路径事后保证(致谢 [openai/codex](https://github.com/openai/codex) 的 apply_patch lark grammar)(MOC-194 / MOC-263)
