...
当前版本 **v2.4.0**(详见 [Changelog](CHANGELOG.md) 与 [Releases](https://github.com/Cmochance/codex-app-transfer/releases))。
...
**Codex 26.623+ 显示档位需补齐启用集(MOC-285)**:Codex(codex-cli 0.142.3)起新增用户级持久设置 `enabled-reasoning-efforts`(默认 `low/medium/high/xhigh/ultra`,不含 `none`/`max`,且无可见开关),reasoning 选择器实际显示 = 模型档位 ∩ 该启用集 —— 导致上面给 GLM 等写的 `none`/`max` 升级到该版本后被过滤、档位塌成「Medium」。本项目在每次启动 Codex 前自动把 `reasoning_tiers` 表暴露的档位(`none`/`high`/`max`)并入 `~/.codex/.codex-global-state.json` 的 `enabled-reasoning-efforts` 持久 atom(**union 只增不删、不动用户其它设置**;因选择器是「模型档 ∩ 启用集」,补全启用集对每个模型仍只显其声明档,GPT 仍 `low/medium/high/xhigh`、GLM 才出现 `none/max`,零副作用),还原原生两档显示。
