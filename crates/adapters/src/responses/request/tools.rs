...
pub(crate) const APPLY_PATCH_TOOL_DESCRIPTION_FOR_CHAT: &str = concat!(
    "Edit files using the apply_patch tool. Use this tool for ANY file content ",
    "change — creating, modifying, refactoring, or deleting code. Do NOT shell out ",
    "to `sed`, `awk`, `cat <<EOF`, `python -c`, `echo > file`, or `printf > file` for ",
    "edits; those bypass the diff UI and frequently leave whitespace mismatches. ",
    "Reserve shell tools for reads (`cat`, `sed -n '1,80p' <path>`), execution, and tests.\n\n",
    ...
    "Every `-` line MUST match the file's current line byte-for-byte — same leading whitespace, ",
    "no trimmed trailing spaces, exact characters. If you are not certain what the line looks like, ",
    "first run `cat <path>` or `sed -n '1,80p' <path>` via shell to read it, then compose the patch ",
    "from the actual bytes. Guessing or paraphrasing `-` content WILL fail.\n\n",
    ...
    "EXAMPLE 2 — create a brand new file `hello.py` with two lines of content. ",
    "For Add File there are NO hunks and NO `@@` markers; every line of the new file's contents is prefixed with `+`:\n",
    "*** Begin Patch\n",
    "*** Add File: hello.py\n",
    ...
    "1. `-` lines must be byte-exact to file content (see BYTE-EXACT MATCHING above). When in doubt, read the file via shell first.\n",
    "2. Empty-line `@@` anchors only work if a blank line actually exists at that position in the file. ",
    "Prefer a non-empty `@@ <header>` anchor (e.g. `@@ def add(a, b):`) over forcing a blank line.\n",
    "3. Do NOT combine `*** Add File: foo` and `*** Update File: foo` in the SAME patch — Update reads the file before Add lands on disk. ",
    "Either make Add File write the final content in one shot, or split into two separate patches.\n",
    "4. `*** Update File:` cannot operate on a completely empty file. Use shell to write at least one line first, then apply_patch.\n",
    "5. In a multi-line file, lone `+` lines AFTER an `@@` anchor APPEND below the anchor — they do NOT replace the anchor line. ",
    "To change a line, use `-` to remove the old line AND `+` to add the new one; do not omit the `-`.\n\n",
    "REMEMBER: the `input` value MUST be a V4A patch enclosed in `*** Begin Patch` / `*** End Patch`. ",
    "Do NOT pass raw source code (e.g. `def main():\\n    pass`) — that is not a valid hunk header and will be rejected."
);
...
pub(crate) const APPLY_PATCH_INPUT_DESCRIPTION_FOR_CHAT: &str = concat!(
    ...
    "`-` lines MUST be byte-exact to the file's current content (read via `cat <path>` first if unsure); ",
    "guessing or paraphrasing them is the #1 cause of `Failed to find expected lines`. ",
    "Chat-path gotchas: prefer non-empty `@@ <header>` anchors over forcing blank-line anchors; ",
    ...
    "NEVER pass raw source code — only a V4A patch envelope is accepted."
);
