...
pub(crate) const APPLY_PATCH_TOOL_DESCRIPTION_FOR_CHAT: &str = concat!(
    ...
    "or `*** Delete File: <path>`. ",
    "Within Update hunks, use single-sided `@@ <header>` markers (no trailing `@@`), ",
    "prefix unchanged context lines with a single space, removed lines with `-`, ",
    ...
    "+    return a / b\n",
    "*** End Patch\n",
    "Notice: no `@@` line at all — 2 lines of space-prefixed context above the `-` ",
    "line already uniquely identify where to apply. Use this whenever you have ",
    "unique context.\n\n",
...
pub(crate) const APPLY_PATCH_INPUT_DESCRIPTION_FOR_CHAT: &str = concat!(
    ...
    "Update hunks use single-sided `@@ <header>` markers (e.g. `@@ class Foo`, `@@ def bar():`) ",
    "with ` `/`+`/`-` line prefixes. **NEVER write a trailing `@@`** ",
    "(`@@ <header> @@` is wrong — Codex Desktop applier treats trailing `@@` as ",
    "literal text and fails with `Failed to find context '... @@'`). ",
    "`@@ <header>` is OPTIONAL when 3 lines of surrounding context already pin the location. ",
    "Add File uses NO `@@` markers — prefix EVERY line of new content with `+` ",
    "(blank lines as bare `+`). Relative paths only. ",
    "`-` lines and space-prefixed context lines MUST be byte-exact to the file's current content ",
