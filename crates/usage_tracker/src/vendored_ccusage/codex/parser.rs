...
// upstream: rust/crates/ccusage/src/adapter/codex/parser.rs
use super::super::{
    date_utils::TimestampMs,
    error::Result,
    types::{CodexRawUsage, CodexTokenUsageEvent},
};
...
pub fn visit_codex_session_file(
    sessions_dir: &Path,
    path: &Path,
...
fn file_modified_timestamp(path: &Path) -> String {
    fs::metadata(path)
        ...
        .map(|duration| {
            super::super::date_utils::format_rfc3339_millis(TimestampMs::from_millis(
                duration.as_millis().min(i64::MAX as u128) as i64,
            ))
        })
        .unwrap_or_else(|| super::super::date_utils::format_rfc3339_millis(TimestampMs::UNIX_EPOCH))
}
