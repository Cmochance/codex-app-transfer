...
4. **任务处理与 PR 全生命周期流程（必须严格遵循）：**
   ...
   **e. 用户显式声明 merge → rebase + 完整清理**
   ...
   - **e2. Merge + 远端 silent delete verify**：`gh pr merge <PR#> --squash --delete-branch` 后必须 `git ls-remote --heads origin <branch>` 验证返 0（remote 真删）；残留时手动 `git push origin --delete <branch>`（worktree 锁本地分支时 gh 也 skip remote delete，silent failure）。
   ...
   - **e6. 关联 issue + followup tracker 更新**：`gh issue view <ISSUE#> --json state,closedByPullRequestsReferences` 验证是否被 PR `Closes #N` 自动关，否则手动 `gh issue close <N>`。**followup tracker 跟 GitHub issue 是两套独立系统**：`docs/followup-tracker.md` 必须手动维护，把本次 PR 实施的 followup 索引行从 Active 移到 Resolved + 详情文件 frontmatter `status: active → resolved` + 加 `resolved_pr` `resolved_date`。如果 main 受保护，改用 `chore: mark followup #N resolved (post-#PR)` 微 PR 提交。
